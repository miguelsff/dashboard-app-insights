...
```bash
npm run dev          # start dev server (uses PORT from .env if set)
npm run build        # production build — runs TypeScript + ESLint checks
npm run lint         # ESLint only
npm run start        # serve the production build locally

bash install.sh      # docker build + run locally (reads PORT from .env)
bash deploy.sh       # build → push to ACR → create/update Azure Web App
bash stop.sh         # stop local Docker container only
bash stop.sh --azure # also stop the Azure Web App
bash stop.sh --delete # delete Azure Web App + App Service Plan (prompts confirmation)
```
...
**Single-page static dashboard.** All data is embedded at build time — no API routes, no server-side fetching, no database.

### Data flow

```
src/data/telemetry.ts  ──►  components  ──►  src/app/page.tsx
```

`src/data/telemetry.ts` is the single source of truth. It exports:
- `telemetryData` — all `TelemetryRecord` objects (typed in `src/types/telemetry.ts`)
- Pre-calculated scalars: `totalRequests`, `successCount`, `failureCount`, `avgDuration`
- Pre-shaped chart arrays: `requestsByOperation`, `requestsByBucket`, `successFailureData`, `timelineData`
- `failedRecords` — failed dependency records

**To add or refresh data:** edit `telemetryData` in `src/data/telemetry.ts`. All derived exports recalculate automatically at module load.

### Client vs Server components

Components that use **Recharts** or **React state** must declare `"use client"` — Recharts depends on `ResizeObserver` (browser-only) and will throw during SSR without it.

- **Client:** `OperationsBarChart`, `PerformanceBucketChart`, `SuccessFailurePieChart`, `RequestTimeline`, `RequestsTable`
- **Server (no directive):** `ResourceInfoCard`, `KpiCards`, `ErrorsPanel`

### Styling conventions

- Dark theme is forced via `className="dark"` on `<html>` in `src/app/layout.tsx`.
- Custom Tailwind tokens: `surface-{600,700,800,900}` (dark backgrounds), `azure-{400,500,600}` (primary accent) — defined in `tailwind.config.ts`.
- Shared component classes (`.card`, `.card-title`, `.badge-success`, `.badge-failure`) live in `src/app/globals.css` under `@layer components`.

### Deployment

- `next.config.mjs` uses `output: "standalone"` — required so Docker can run `.next/standalone/server.js`.
- Dockerfile is a 3-stage build: `deps` (npm ci) → `builder` (next build) → `runner` (standalone server).
- `PORT` in `.env` controls the container port and the Azure Web App `WEBSITES_PORT` setting.
- `deploy.sh` requires `az` CLI and `docker` to already be authenticated before running.
